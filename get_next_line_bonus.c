...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2023/02/07 17:28:33 by mthabit           #+#    #+#             */
/*   Updated: 2023/02/12 16:38:04 by mthabit          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
char	*get_next_line(int fd)
{
	...
	line = NULL;
	store[fd] = read_file(fd, store[fd]);
	line = take_line(store[fd]);
	if (line && !*line)
		return (NULL);
	store[fd] = updated_data(store[fd]);
	return (line);
}
